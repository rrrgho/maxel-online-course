 <!-- Tracker section start -->
 <section class="about-style-2 mt-5" id="about-us">
     <div class="container custom-container">
         <div class="row justify-content-lg-between justify-content-center gy-md-5 gy-3">
             <div class="col-lg-6">
                 <div class="title-style-1">
                     <h5 class="txt-success"># Everything is transparent</h5>
                     <h2>Track Your development progress</h2>
                 </div>
                 <div class="about-content">
                     <p>
                         We make it easier and much more transparent to our client track their System Development
                         Progress.
                         Doiscode providing premium Timeline Tracker which can help our client monitor the progress and
                         keep on timeline
                     </p>
                     <figure>
                         <blockquote class="blockquote" style="border-color: #FF4E7E !important;">
                             <p>"We allow client to Live interact to our dev in every tickets, so it makes both parties
                                 easier to understand more needs."</p>
                         </blockquote>
                     </figure>
                     <!-- <button type="button" class="btn btn-primary rounded-pill">See More Tracking</button> -->
                 </div>
             </div>
             <div class="col-xl-5 col-lg-6 col-sm-8 col-10">
                 <div class="about-image">
                     <img class="img-fluid" src="{{asset('assets/images/app/about.webp')}}" alt="mobile">
                 </div>
             </div>
         </div>
     </div>
 </section>
 <!-- Tracker section end -->
