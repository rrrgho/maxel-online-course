    ...
    public function classLeassonList($class_id, $leasson_id){
        ...
        if($class->type === 1){
            if($class->your_class !== 'APPROVED')
                return redirect()->back()->with('error', 'You have no access to this class, Buy now !');
        }else{
            if(!Auth::user()->basic_user)
                return redirect()->back()->with('error', 'You have no access to this basic class, Buy now !');
        }
        $leasson = ClassLeasson::find($leasson_id);
        $leasson_completed = LeassonComplete::where([
            ['class_id', $class_id],
            ['user_id', Auth::user()->id],
        ])->count();
        return view('User.BasicClassLeassonList.index', ['class' => $class, 'leasson' => $leasson, 'leasson_completed' => $leasson_completed]);
    }
